namespace :warper do
  namespace :old_users do
    ...
    task :plan_purge, %i[limit updated_at] => :environment do |_t, args|
      ...
      users = User.where('own_maps_count > 0').order('own_maps_count DESC NULLS LAST').where('updated_at < ?',
                                                                                             before_updated_at).limit(args.limit)
      ...
      users.each do |user|
        ...
        puts "#{user.login}, #{user.email}, #{user.updated_at.strftime('%F')}, #{user.created_at.strftime('%F')}, #{user.own_maps_count}, #{number_to_human_size(user.upload_filesize_sum)}, #{if user.disk_usage && user.disk_usage > 0
                                                                                                                                                                                                 number_to_human_size(user.disk_usage * 1024)
                                                                                                                                                                                               end} "
      end
    ...
    task :notify_purge, %i[limit updated_at] => :environment do |_t, args|
      include ActiveSupport::NumberHelper
