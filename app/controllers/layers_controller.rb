class LayersController < ApplicationController
  layout 'layerdetail', only: %i[show edit export metadata]
  before_action :authenticate_user!,
                except: %i[wms wms2 show_kml show index metadata maps thumb geosearch comments tile trace id]
  before_action :check_administrator_role, only: %i[publish toggle_visibility merge trace id]

  before_action :find_layer,
                only: %i[show export metadata toggle_visibility update_year publish remove_map merge maps thumb
                         comments trace id]
  before_action :check_if_layer_is_editable, only: %i[edit update remove_map update_year update destroy]
  ...
  def thumb
    ...
    if ['missing.png', 'private.png'].include?(@layer.thumb)
  ...
  def index
    ...
    if request.xhr?
      ...
      respond_to do |format|
        ...
        format.xml do
          render xml: @layers.to_xml(root: 'layers',
                                     except: %i[uuid parent_uuid
                                                description]) { |xml|
        ...
        format.json do
          render json: { stat: 'ok', items: @layers.to_a }.to_json(except: %i[uuid parent_uuid description]),
  ...
  def maps
    ...
    lmaps = if show_warped == '0'
              @layer.maps.order(:map_type).paginate(paginate_params)
            else
              @layer.maps.warped.order(:map_type).paginate(paginate_params)
            end
    respond_to do |format|
      ...
      format.json do
        render json: { stat: 'ok',
                       ...
                       items: lmaps.to_a }.to_json(except: %i[content_type size bbox_geom uuid parent_uuid filename parent_id map thumbnail]), callback: params[:callback]
      end

      format.xml do
        render xml: lmaps.to_xml(root: 'maps',
                                 except: %i[content_type size bbox_geom uuid
                                            parent_uuid filename parent_id map thumbnail]) { |xml|
  ...
  def show
    ...
    if request.xhr?
      ...
      respond_to do |format|
        ...
        format.json do
          render json: { stat: 'ok', items: @layer }.to_json(except: %i[uuid parent_uuid description]),
                 callback: params[:callback]
        end
        format.xml { render xml: @layer.to_xml(except: %i[uuid parent_uuid description]) }
  ...
  def edit
    ...
    @maps = if !current_user.own_this_layer?(params[:id]) and current_user.has_role?('editor')
              @layer.user.maps
            else
              current_user.maps # current_user.maps.warped
            end
  ...
  def toggle_visibility
    ...
    @layer.save
    @layer.update_layer
    update_text = if @layer.is_visible
  ...
  def wms2
    ...
    begin
      ...
      %i[request version transparency service srs width height bbox format srs layers].each do |key|
        ows.setParameter(key.to_s, ok_params[key.to_s.upcase]) unless ok_params[key.to_s.upcase].nil?
      end
      ...
      if @layer_name
        ...
        Layer.visible.each do |layer|
          next unless layer.rectified_maps_count > 0
  ...
  def store_location
    anchor = case request.parameters[:action]
             when 'metadata'
               "#{t('layouts.tabs.metadata')}_tab"
             when 'export'
               "#{t('layouts.tabs.export')}_tab"
             else
               ''
             end
    session[:return_to] = if request.parameters[:action] && request.parameters[:id]
                            layer_path(id: request.parameters[:id], anchor: anchor)
                          else
                            request.request_uri
                          end
  end
