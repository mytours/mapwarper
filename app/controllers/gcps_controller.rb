class GcpsController < ApplicationController
  ...
  before_action :authenticate_user!,
                only: %i[update update_field add destroy add_many add_many_to_map csv]
  before_action :check_editor_role, only: %i[add_many bulk_import]
  before_action :check_administrator_role, only: [:csv]
  before_action :find_gcp, only: %i[show update update_field destroy]
  ...
  def update
    ...
    respond_to do |format|
      if @gcp.update_attributes(x: x, y: y, lon: lon, lat: lat, name: name, soft: soft)
        ...
        format.json do
          render json: { stat: 'ok', items: @gcps.to_a }.to_json(methods: :error), callback: params[:callback]
        end
        format.xml { render xml: @gcps.to_xml(methods: :error) }
      else

        format.json do
          render json: { stat: 'fail', message: 'Could not update GCP', errors: @gcp.errors.to_a, items: [] }.to_json,
  ...
  def update_field
    ...
    respond_to do |format|
      if @gcp.update_attribute(attribute, value)
        ...
        format.js { render action: 'update' } if request.xhr?
        format.html { redirect_to_index }
        format.json do
          render json: { stat: 'ok', items: @gcps.to_a }.to_json(methods: :error), callback: params[:callback]
        end
  ...
  def destroy
    ...
    respond_to do |format|
      if @gcp.destroy
        ...
        format.json do
          render json: { stat: 'fail', message: 'Could not delete GCP', errors: @gcp.errors.to_a, items: [] }.to_json,
                 callback: params[:callback]
        end
      end
    end
  ...
  def add
    ...
    @gcp = if params[:mapid]
             Gcp.new(map_id: params[:mapid].to_i, x: x, y: y, lat: lat, lon: lon, name: name,
                     soft: soft)
           else
             Gcp.new(x: x, y: y, lat: lat, lon: lon)
           end
    ...
    respond_to do |format|
      if @gcp.save
        ...
        format.json do
          render json: { stat: 'fail', message: 'Could not add GCP', errors: @gcp.errors.to_a, items: [] }.to_json,
                 callback: params[:callback]
        end
      end
    end
  ...
  def add_many
    ...
    begin
      ...
      respond_to do |format|
        ...
        format.json do
          render json: { stat: "#{t('.not_found')} #{e.message}", items: [] }.to_json, status: :not_found
        end
      end
      return false
    end
  ...
  # 1.1,2.1,3.2,3.2,foo
  def add_many_to_map
    Gcp.add_many_from_file(params[:file], params[:mapid]) if params[:file]
