class UsersController < ApplicationController
  ...
  before_action :check_super_user_role,
                only: %i[index destroy enable disable stats disable_and_reset force_confirm]
  ...
  def show
    ...
    respond_to do |format|
      ...
      format.js {}
      format.json do
        render json: { stat: 'ok', items: @user.to_a }.to_json(only: %i[login created_at stat items enabled])
      end
    end
  end
