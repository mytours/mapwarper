class Api::V1::ActivityController < Api::V1::ApiController
  ...
  def stats
    ...
    sort_key = %w[total_count map_count gcp_count gcp_update_count gcp_create_count gcp_destroy_count username
                  user_id].detect do |f|
      f == params[:sort_key]
    end
    sort_key ||= 'total_count' if sort_order == 'desc'
  ...
  def render_json(audits)
    render json: { data: audits, meta: { 'total_entries' => audits.total_entries, 'total_pages' => audits.total_pages } },
           adapter: :json
  end
end
