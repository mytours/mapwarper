class Api::V1::ApiController < ActionController::API
  ...
  def missing_param_error(exception)
    render json: { errors: [{ title: 'Missing param', detail: exception.message }] },
  ...
  def validate_jsonapi_type
    ...
    unless params[:data] && params[:data][:type]
      render json: { errors: [{ title: 'Missing param', detail: 'params data or type is missing' }] },
             status: :unprocessable_entity
      return
    end
    return unless params[:data][:type] != type

    render json: { errors: [{ title: 'Invalid param', detail: "params type should be '#{type}'" }] },
           status: :unprocessable_entity
    nil
  end
  ...
  def index
    ...
    m = Redcarpet::Markdown.new(renderer,
                                ...
                                autolink: true,
                                tables: true)
    content = m.render(File.read(Rails.root + 'README_API.md'))
  ...
  #
  def check_protocol
    Rails.application.routes.default_url_options[:protocol] = if request.ssl?
                                                                'https'
                                                              else
                                                                'http'
                                                              end
