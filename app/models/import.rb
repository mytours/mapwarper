class Import < ActiveRecord::Base
  ...
  validates :metadata, presence: { message: :no_file_uploaded }

  acts_as_enum :status, %i[ready running finished failed]
  ...
  def log_path
    "#{Rails.root.join("log/imports/#{log_filename}")}"
  end
  ...
  def finish_import(_options)
    ...
    save
    save_maps_to_layers unless maps.empty? || save_layer == false || layer_ids.empty?
  ...
  def import_maps
    data = IO.read(metadata.path).scrub # scrub converts to utf8 encoding
    ...
    map_data.each do |map_row|
      ...
      if map.save
        ...
        Import.increment_counter(:imported_count, id)
      else
  ...
  def save_maps_to_layers
    ...
    layers.each do |layer|
      ...
      maps_to_add = maps.select { |map| !layer.maps.include?(map) }
  ...
  def delete_logfile
    if log_filename && log_filename.include?('.log') && File.exist?("#{Rails.root.join("log/imports/#{log_filename}")}")
      File.delete("#{Rails.root.join("log/imports/#{log_filename}")}")
    end
  end
