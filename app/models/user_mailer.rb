class UserMailer < ActionMailer::Base
  ...
  def new_registration(user)
    @user = user
    @subject = I18n.t('user_mailer.new_registration.subject', site_name: APP_CONFIG['site_name'],
  ...
  def old_user_notify(user)
    @user = user
    @subject = I18n.t('user_mailer.old_user_notify.subject', site_name: APP_CONFIG['full_site_name'],
                                                             user_name: @user.login, map_count: @user.own_maps_count)
