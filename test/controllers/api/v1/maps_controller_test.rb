...
class MapsControllerTest < ActionController::TestCase
  ...
  class SingleMapTest < MapsControllerTest
    def cleanup_images(map)
      File.delete(map.temp_filename) if File.exist?(map.temp_filename)
      File.delete(map.warped_filename) if File.exist?(map.warped_filename)
      File.delete(map.warped_png_filename) if File.exist?(map.warped_png_filename)
    ...
    def cleanup_gml(map)
      return if map.masking_file_gml == Rails.root.join('test/fixtures/data//test').to_s + '.gml'

      File.delete(map.masking_file_gml) if File.exist?(map.masking_file_gml)
      File.delete(map.masking_file_gml + '.ol') if File.exist?(map.masking_file_gml + '.ol')
      return unless File.exist?(map.masking_file_gfs)
    ...
    class LoggedOut < SingleMapTest
      test 'get a map' do
        get :show, params: { id: @map.id, format: :json }
        assert_response :success
        assert_not_nil assigns(:map)
      end

      test 'get map with bbox' do
        get :show, params: { id: @warped_map.id, format: :json }
        assert_response :success
        assert_not_nil assigns(:map)
      ...
      test 'get_status' do
        get 'status', params: { id: @map.id, format: :json }
        assert_response :ok
    ...
    class LoggedIn < SingleMapTest
      ...
      test 'update map when they own it' do
        params = { :id => @warped_map.id, :format => :json,
                   'data' => { 'type' => 'maps', 'attributes' => { 'title' => 'foojson' } } }
      ...
      test 'not create map basic without a title ' do
        assert_difference('Map.count', 0) do
          post 'create',
               params: { :format => :json,
                         'data' => { 'type' => 'maps',
                                     'attributes' => { 'description' => 'foo', 'wrong' => 'new map' } } }
        end
      ...
      test 'create map basic' do
        assert_difference('Map.count', 1) do
          post 'create',
               params: { :format => :json,
                         'data' => { 'type' => 'maps',
                                     'attributes' => { 'description' => 'foo', 'title' => 'new map' } } }
        end
      ...
      test 'create map from file upload' do
        image_data = Base64.encode64(File.binread(Rails.root.join('test/fixtures/data/100x70map.png').to_s))
        ...
        assert_difference('Map.count', 1) do
          post 'create',
               params: { :format => :json,
                         'data' => { 'type' => 'maps',
                                     'attributes' => { 'description' => 'poo', 'title' => 'new map', 'upload' => upload,
                                                       'upload_file_name' => '100x70map2.png' } } }
        end
        assert_response :created
        body = JSON.parse(response.body)
      ...
      test 'create map from URL' do
        stub_request(:get, 'http://example.com/100x70map.png')
          .to_return(
            status: 200,
            body: File.read(Rails.root.join('test/fixtures/data/100x70map.png').to_s),
        ...
        assert_difference('Map.count', 1) do
          post 'create',
               params: { :format => :json,
                         'data' => { 'type' => 'maps',
                                     'attributes' => { 'description' => 'poo', 'title' => 'new map', 'upload_url' => 'http://example.com/100x70map.png' } } }
        end
        assert_response :created
        body = JSON.parse(response.body)
        assert_equal 'new map', body['data']['attributes']['title']
      ...
      test 'rectify no gcps' do
        patch :rectify, params: { id: @warped_map.id, format: :json, warp: 1 }
        assert_response :unprocessable_entity
        body = JSON.parse(response.body)
      ...
      test 'save_mask' do
        test_gml = Rails.root.join('test/fixtures/data/', @warped_map.id.to_s).to_s + '.gml'
        Map.any_instance.stubs(:masking_file_gml).returns(test_gml)

        assert_not File.exist?(test_gml)
        ...
        post 'mask', params: { id: @warped_map.id, format: :json, output: output }
        assert_response :ok
        assert File.exist?(test_gml)
      ...
      test 'delete_mask' do
        ...
        delete 'delete_mask', params: { id: @warped_map.id, format: :json }
        assert_response :ok
      ...
      # clip (apply mask)
      test 'clip_mask' do
        test_gml = Rails.root.join('test/fixtures/data//test').to_s + '.gml'
        Map.any_instance.stubs(:masking_file_gml).returns(test_gml)
        test_gfs = Rails.root.join('test/fixtures/data//test').to_s + '.gfs'
        Map.any_instance.stubs(:masking_file_gfs).returns(test_gfs)

        patch 'crop', params: { id: @warped_map.id, format: :json }
      ...
      test 'mask_crop_rectify' do
        ...
        output = '<wfs:FeatureCollection xmlns:wfs="http://www.opengis.net/wfs"><gml:featureMember xmlns:gml="http://www.opengis.net/gml"><feature:features xmlns:feature="http://mapserver.gis.umn.edu/mapserver" fid="OpenLayers.Feature.Vector_207"><feature:geometry><gml:Polygon><gml:outerBoundaryIs><gml:LinearRing><gml:coordinates decimal="." cs="," ts=" ">1490.0376070686068,5380.396178794179 3342.4880893970894,5380.214910602912 3582.659,5126.446 3555.463,4813.692 3637.051,4487.34 4276.157,3753.048 4575.313,3113.942 4546.465124740124,1412.519663201663 2417.4615530145525,1317.354124740125 1431.415054054054,1294.9324823284824 1447.7525384615387,2187.807392931393 1434.5375363825372,5034.563750519751 1490.0376070686068,5380.396178794179</gml:coordinates></gml:LinearRing></gml:outerBoundaryIs></gml:Polygon></feature:geometry></feature:features></gml:featureMember></wfs:FeatureCollection>'
        patch 'mask_crop_rectify', params: { id: @map.id, format: :json, output: output }
        assert_response :ok
    ...
    class LoggedInAdmin < SingleMapTest
      ...
      test 'publish' do
        ...
        patch 'publish', params: { id: @warped_map.id, format: :json }
        assert_response :ok
        body = JSON.parse(response.body)
      ...
      test 'unpublish' do
        ...
        @warped_map.save
        patch 'unpublish', params: { id: @warped_map.id, format: :json }
        assert_response :ok
        body = JSON.parse(response.body)
        assert_equal 'warped', body['data']['attributes']['status']
      end
  ...
  class CollectionMapTest < MapsControllerTest
    ...
    test 'get list of maps' do
      get :index, params: { foo: 'bar', format: :json }
      assert_response :success
      assert_not_nil assigns(:maps)
      body = JSON.parse(response.body)
    ...
    test 'search maps' do
      get :index, params: { query: 'title', format: :json }
      assert_response :success
      assert_not_nil assigns(:maps)
      body = JSON.parse(response.body)
    ...
    test 'sort maps' do
      get :index, params: { sort_order: 'asc', sort_key: 'title', format: :json }
      assert_response :success
      assert_not_nil assigns(:maps)
      body = JSON.parse(response.body)
      assert_equal 7, body['data'].length
    ...
    test 'sort and query maps' do
      get :index, params: { query: 'map', sort_order: 'asc', sort_key: 'title', format: :json }
      assert_response :success
      assert_not_nil assigns(:maps)
      body = JSON.parse(response.body)
    ...
    test 'get warped' do
      get :index, params: { show_warped: '1', format: :json }
      assert_response :success
      assert_not_nil assigns(:maps)
      body = JSON.parse(response.body)
      assert_equal 1, body['data'].length
    ...
    test 'geosearch' do
      ...
      get :index,
          params: { bbox: '26.849937539609, 58.328018921793, 26.779899697812, 58.421402710855', operation: 'intersect',
                    format: :json }
      assert_response :success
      assert_not_nil assigns(:maps)
      body = JSON.parse(response.body)
      ...
      # within, so expect 0
      get :index,
          params: { bbox: '26.849937539609, 58.328018921793, 26.779899697812, 58.421402710855', operation: 'within',
                    format: :json }
      assert_response :success
      ...
      get :index,
          params: { bbox: '26.633644204648, 58.302613038003, 26.859550576718, 58.428269165933', operation: 'intersect',
                    format: :json }
      assert_response :success
      assert_not_nil assigns(:maps)
      body = JSON.parse(response.body)
      ...
      # within, so expect 0
      get :index,
          params: { bbox: '26.633644204648, 58.302613038003, 26.859550576718, 58.428269165933', operation: 'within',
                    format: :json }
      assert_response :success
      assert_not_nil assigns(:maps)
      body = JSON.parse(response.body)
    ...
    test 'geosearch with query' do
      get :index,
          params: { bbox: '26.849937539609, 58.328018921793, 26.779899697812, 58.421402710855', query: 'title',
                    operation: 'intersect', format: :json }
      assert_response :success
      assert_not_nil assigns(:maps)
      body = JSON.parse(response.body)
      assert_equal 'title', body['data'][0]['attributes']['title']
      ...
      get :index,
          params: { bbox: '26.849937539609, 58.328018921793, 26.779899697812, 58.421402710855', query: 'map',
                    operation: 'intersect', format: :json }
      assert_response :success
      assert_empty assigns(:maps)
      body = JSON.parse(response.body)
