...
class LayersControllerTest < ActionController::TestCase
  ...
  class CollectionTest < LayersControllerTest
    ...
    test 'sorting' do
      get 'index', params: { sort_key: 'name', sort_order: 'asc' }
      assert_response :ok
      assert_not_nil assigns(:layers)
      body = JSON.parse(response.body)
      assert_equal Layer.count, body['data'].length
    ...
    test 'percent sorting' do
      get 'index', params: { sort_key: 'percent', sort_order: 'desc' }
      assert_response :ok
      assert_not_nil assigns(:layers)
      body = JSON.parse(response.body)
    ...
    test 'geosearch with query' do
      get :index,
          params: { bbox: '26.849937539609, 58.328018921793, 26.779899697812, 58.421402710855', query: 'name',
      ...
      get :index,
          params: { bbox: '26.849937539609, 58.328018921793, 26.779899697812, 58.421402710855', query: 'layer',
                    operation: 'intersect', format: :json }
      assert_response :success
      assert_empty assigns(:layers)
      body = JSON.parse(response.body)
  ...
  class NormalUserTest < LayersControllerTest
    ...
    test 'update' do
      patch 'update',
            params: { :id => @layer.id, 'data' => { 'type' => 'layers', 'attributes' => { name: 'updated layer' } } }
      assert_response :ok
      body = JSON.parse(response.body)
    ...
    test 'update with maps' do
      ...
      patch 'update',
            params: { :id => @layer.id,
                      'data' => { 'type' => 'layers', 'attributes' => { name: 'updated layer' },
    ...
    test 'delete' do
      assert_difference('Layer.count', -1) do
        delete 'destroy', params: { id: @layer.id }
      end
      assert_response :success
    end
    ...
    test 'remove map' do
      ...
      patch 'remove_map', params: { id: @layer.id, map_id: map_id }
    ...
    test 'create' do
      assert_difference('Layer.count', 1) do
        post 'create',
             params: { 'data' => { 'type' => 'layers', 'attributes' => { name: 'new layer', description: 'bar' } } }
      end
  ...
  class MemberTest < LayersControllerTest
    ...
    test 'get layer' do
      get 'show', params: { id: @layer.id, format: :json }
      assert_response :ok
      assert_response :success
    ...
    # visibility (patch)
    test 'visible' do
      patch 'toggle_visibility', params: { id: @layer.id }
      assert_response :ok
      body = JSON.parse(response.body)
    ...
    test 'merge' do
      ...
      patch 'merge', params: { id: @layer.id, dest_id: dest_layer.id }
