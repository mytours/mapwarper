...
class GcpsControllerTest < ActionController::TestCase
  ...
  test 'create with no map id' do
    assert_difference('Gcp.count', 0) do
      post 'create', params: { 'data' => { 'type' => 'gcps', 'attributes' => { 'x' => 1 } } }
    end
  ...
  test 'create' do
    assert_difference('Gcp.count', 1) do
      post 'create',
           params: { 'data' => { 'type' => 'gcps',
                                 'attributes' => { x: 1, y: 2, lat: 33.3, lon: 44.4, map_id: @map.id } } }
    end
  ...
  test 'update' do
    patch 'update', params: { :id => @gcp.id, 'data' => { 'type' => 'gcps', 'attributes' => { 'x' => 99 } } }
    assert_response :ok
  ...
  test 'delete' do
    assert_difference('Gcp.count', -1) do
      delete 'destroy', params: { id: @gcp.id }
    end
    assert_response :ok
  ...
  test 'add_many' do
    sign_out @user
    editor_user_sign_in
    post 'add_many', params: { gcps: [{ 'mapid' => @map.id, 'x' => 1.2, 'y' => 2.2, 'lat' => 11.1, 'lon' => 21.1 },
                                      { 'mapid' => @map.id, 'x' => 22, 'y' => 33, 'lat' => 55.1, 'lon' => 55.1 }] }
    assert_response :ok
    body = JSON.parse(response.body)
